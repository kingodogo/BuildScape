...
public class PetRenderer extends MobRenderer<PetEntity, PetModel<PetEntity>> {
    ...
    public PetRenderer(EntityRendererProvider.Context context) {
        super(context, new PetModel<>(context.bakeLayer(PetModel.LAYER_LOCATION)), 0.2F);

        final net.minecraft.client.renderer.entity.ItemRenderer itemRenderer = Minecraft.getInstance().getItemRenderer();
        this.addLayer(new RenderLayer<PetEntity, PetModel<PetEntity>>(this) {
            @Override
            public void render(PoseStack poseStack, net.minecraft.client.renderer.MultiBufferSource buffer, int packedLight, PetEntity entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
    ...
    public static void registerRenderers(EntityRenderersEvent.RegisterRenderers event) {
        event.registerEntityRenderer(ModEntities.PET.get(), PetRenderer::new);
    }
